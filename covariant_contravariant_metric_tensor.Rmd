...
$$A = \begin{bmatrix}\mathbf e_1 \cdot \mathbf e_1 & \mathbf e_1 \cdot \mathbf e_2 \\
\mathbf e_2 \cdot \mathbf e_1 & \mathbf e_2 \cdot \mathbf e_2\end{bmatrix} =\begin{bmatrix} 1.64 & 0.55 \\ 0.55 & 1.06 \end{bmatrix}$$

with 

$$A^{-1} = \begin{bmatrix}\mathbf e^1 \cdot \mathbf e^1 & \mathbf e^1 \cdot \mathbf e^2 \\
\mathbf e^2 \cdot \mathbf e^1 & \mathbf e^2 \cdot \mathbf e^2\end{bmatrix} =\begin{bmatrix} 0.74 & -0.38 \\ -0.38 & 1.14 \end{bmatrix}$$
...
<img src="covariant_movie.gif" width=600>

---

See also [this post](https://physics.stackexchange.com/a/821513/150551) around the animated graphic above:

Given the [metric tensor for the basis vectors][1]
...
$$\begin{align}
\varepsilon^1 &= 3.35 \mathbf e_1 -1.77  \mathbf e_2=[1.43,1.14]^\top\\
\varepsilon^2 &= -1.77 \mathbf e_1 +7.71  \mathbf e_2= [-2.38, 1.43]^\top
\end{align}$$
...
$$[e^1 , e^2]\begin{bmatrix} x \\
y\end{bmatrix}=e^1x + e^2y = \text{constant }n \in \mathbb Z \implies \begin{cases}
y = \frac{n - 1.43x}{1.14}\color{magenta}{\bf\text{ \\\\}} \\
y = \frac{n + 2.38x}{1.43} \color{purple}{\bf\text{ //}}
\end{cases}$$
...
The dashed red and blue arrows demonstrate the mutual orthogonality of the components in the contravariant and covariant bases: in the contravariant (parallel to axes) original basis, the construction (in red) is orthogonal to the cobasis vectors; in the covariant basis (perpendicular to axes) construct, the lines (in blue) are orthogonal to the lines spanned by the original basis vectors.
  ...
  [1]: https://www.seas.upenn.edu/~amyers/DualBasis.pdf
