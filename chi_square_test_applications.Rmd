...
The grade distribution in a test with grades distributed normally should follow the frequencies:

```{r}
grades_Z_scores <- c("-2sd to -1sd", "-1sd to 0", 
            "0 to 1sd", "1sd to 2sd")
minus_2_to_1 <- pnorm(-1) - pnorm(-2)
minus_1_to_0 <- pnorm(0) - pnorm(-1)
exp_frequency <- c(minus_2_to_1,minus_1_to_0,
                   minus_1_to_0,minus_2_to_1)
# using the symmetry of the normal distribution.

(N <- data.frame(grades_Z_scores, exp_frequency))
```

If the observed frequencies are:
