...
Since it is linear combinations of columns, it is in $\mathbb R^m$

####NULL SPACE of $A$ or $N(A)$:

Solutions of $Ax=0$. Hence the null space is in $\mathbb R^n.$

####ROW SPACE $R(A)$: All combinations of the columns of $A^\top$, i.e. $C(A^\top)$

It lives in $\mathbb R^n$.

####NULL SPACE of $A^\top$, i.e. $N(A^\top)$ - THE LEFT NULL SPACE OF A:

It is in $\mathbb R^m$ because the null of $A$ is in $\mathbb R^n.$
