...
&=\mathbb E\left(X^2 \right)\quad-\quad \left[\;\mathbb E\left(X\right)\;\right]^2
\end{align}
$$
...
$$\frac{n}{n-1}\,\left[\,\displaystyle \sum_1^n\frac{x_i^2}{n}  -\left[ \displaystyle \sum_1^n \frac{x_i}{n}\right]^2\right]=
\displaystyle \sum_1^n\,\frac{x_i^2}{n-1}  -\frac{n}{n-1}\left[ \displaystyle \sum_1^n \frac{x_i}{n}\right]^2$$
...
# Taking the estimate of the expectation for each random vector E(V):

mu = rowMeans(X)

# Forming a matrix of means for the next step:

(means = t(cbind(rep(mu[1],ncol(X)),
                 rep(mu[2],ncol(X)),
                 rep(mu[3],ncol(X)))))

# Calculating manually the variance with the alternate formula

((X %*% t(X)) / (ncol(X) - 1)) - (ncol(X)/(ncol(X) - 1))* (means[,1] %*% t(means[,1])) 
