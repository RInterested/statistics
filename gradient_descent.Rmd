...
$\delta^{(4)}=a_j^{(4)}-y_j=(h_\Theta(x))_j-y_j)$ for every $j$ in the layer, and the rest as the same as in the body of this webpage:

$\delta^{(3)}=(\Theta^{(3)})^T\delta^{(4)}\,g'(z^{(3)})$

$\delta^{(2)}=(\Theta^{(2)})^T\delta^{(3)}\,g'(z^{(2)})$

stopping at $\lambda^{(2)}$.

So, he is effectively introducing an additional $\delta^{(4)}$.
