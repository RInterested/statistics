...
plot(values$x, values$value, pch=19, col=as.numeric(values$variable))
```
...
where $X$ are the independent variable of the sample and $X_*$ are values intially employed to build the curves, i.e. not the sample, but just the $x$ axis.
...
fig2b <- ggplot(values2, aes(x=x, y=value)) +
  geom_line(aes(group=variable), colour="grey80", linewidth=1) +
  geom_line(data=values, aes(x=x.star, y=f.star.bar), colour="red", linewidth=2) + # Plots the mean, which depends on x
...
gg <- ggplot(values2, aes(x=x, y=value)) + 
  geom_line(aes(group=variable), colour="grey70") +
  geom_line(data=values, aes(x=x.star,y=f.bar.star),colour="red", linewidth=3) + 
