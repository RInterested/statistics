...
Above $K_\nu$ is a modified Bessel function of the second kind, $\nu$ controls smoothness, and $l$ is a length scale as before. As $\nu →∞$ , i.e., a very smooth parameterization, we get

$$K_\nu (x,x') \to k_\infty(x,x')=\exp\left( -\frac{(\Vert x - x'\Vert)^2}{2l}\right)$$
...
matern <- function(X1, X2, l=1, nu=1/2) {
  Sigma <- matrix(rep(0, length(X1) * length(X2)), nrow=length(X1))
  for (i in 1:nrow(Sigma)) {
    for (j in 1:ncol(Sigma)) {
      Sigma[i,j] <- (2^(1 - nu))/gamma(nu) * (sqrt(X1[i]^2 + X2[j]^2) * sqrt(2*nu/l))^nu * besselK(sqrt(X1[i]^2 + X2[j]^2) * sqrt(2*nu/l), nu)
    }
  }
...
# Define the points at which we want to define the functions
x.st <- seq(0, 5, len = 100)

# Calculate the covariance matrix
sig <- matern(x.st, x.st, 1, 1/2)
...
for (i in 1:n.samples) {
  # Each column represents a sample from a multivariate normal distribution with zero mean and covariance sigma
  fsta[,i] <- mvrnorm(1, rep(0, length(x.st)), sig)
}
val <- cbind(x = x.st, as.data.frame(fsta))
val <- melt(val, id="x")


co <- brewer.pal(n = n.samples, name = 'Spectral')
par(bg='gray5')
ppp <- ggplot(val, aes(x=x, y=value)) +
  geom_line(aes(group=variable), colour=co[as.numeric(val$variable)], linewidth=1) 
ppp
```
