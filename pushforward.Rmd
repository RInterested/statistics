...
$$\bbox[magenta,5px,border:2px solid black]{\color{white}{\phi^*\left( \tau_q\right)= \phi^*  \left(\sum_{a=1}^m \left. c_a\, dy^a \right|_q \right)=\sum_{a=1}^m c_a\, \left( d\phi^a\right)_p = \sum_{i=1}^n\left( \sum_{a=1}^m c_a \left. \frac{\partial \phi^a}{\partial x^i}\right|_p \right)\left. dx^i\right|p\quad}}$$
