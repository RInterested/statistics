...
set.seed(42)
num_paths   <- 40     # Number of individual lines to draw
num_steps   <- 1000   # Number of time steps (flips)

# Ole Peters' exact parameters (Fixed Volatility Regime)
gain_factor <- 1.50   # Heads: +50%
loss_factor <- 0.60   # Tails: -40%

# Single-knob options: We sweep the Probability of Heads (p)
...
for(i in seq_along(prob_options)) {
  ...
  wealth_matrix[1, ] <- 1 # Every player starts with $1
  
  for(t in 1:num_steps) {
    # Generate discrete Bernoulli outcomes for each path simultaneously
    flips <- sample(
  ...
  # 2. Calculate the True Ensemble Expected Line: E[W_t] = W_0 * (E[shocks])^t
  expected_single_shock <- (p_heads * gain_factor) + ((1 - p_heads) * loss_factor)
  ensemble_line <- 1 * (expected_single_shock)^time_vector
  
  # Default view target: Fair coin (p = 0.50) reveals the ergodicity paradox
  ...
  # 4. Add the bold Ensemble Expected Value line
  p <- p %>% add_trace(
    x = time_vector,
    y = ensemble_line,
    type = "scatter", mode = "lines",
...
# 5. Build Restyle Visibility Steps for the Probability Slider
slider_steps <- list()
...
# 6. Render final canvas on a log scale
p %>% layout(
  title = "Ole Peters' Coin Toss Multipliers (+50% / -40%)",
  showlegend = FALSE,
  ...
  sliders = list(
    list(
      active = which(abs(prob_options - 0.50) < 1e-5) - 1, # Defaults to a fair coin (p = 0.50)
