...
Here is an example with $150$ paths:


---

```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=6, fig.height=4, fig.align='center'}
library(plotly)
library(dplyr)

set.seed(42)
num_paths   <- 500    
num_steps   <- 150     # Fixed: Shortened time horizon lets outliers appear in a small sample pool
...
final_values   <- wealth_matrix[num_steps + 1, ]
sorted_indices <- order(final_values, decreasing = TRUE)
...
p <- p %>% add_trace(
  ...
  type = "scatter", mode = "lines+text",
  line = list(color = "rgba(0, 0, 0, 1)", width = 3.5, dash = "dash"),
  text = text_vector, textposition = "top center", 
  font = list(color = "rgba(0, 0, 0, 1)", size = 12, fontfamily = "Arial-Bold"),
  name = "Ensemble Average"
)

p %>% layout(
  title = "Ole Peters' Fair Coin Toss: The Isolated Outliers Driving the Mean (150 Steps)",
  showlegend = FALSE,
  plot_bgcolor = "white",
  xaxis = list(title = "Time Steps (Flips)", gridcolor = "rgba(240,240,240,1)"),
