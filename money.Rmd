...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=8, fig.height=4, fig.align='center'}
library(plotly)
library(dplyr)
...
# 3. Populate Right Subplot (Horizontal Raw Linear Histogram)
# We choose a horizontal orientation ("h") to keep wealth aligned vertically on both plots!
...
p_right <- p_right %>% add_trace(
  x = clipped_wealth, type = "histogram",
  orientation = "v",
  marker = list(color = "purple", line = list(color = "purple", width = 1.5)),
  autobiny = FALSE,
  xbins = list(start = 0, end = max_display, size = max_display / 10),
...
subplot(p_left, p_right, nrows = 1, widths = c(0.65, 0.35), margin = 0.06, shareY = FALSE) %>%
  layout(
    ...
    # Right Subplot Axes (Linear scale reveals the raw geometric asymmetry)
    xaxis2 = list(title = "Count (Paths)", gridcolor = "rgba(240,240,240,1)"),
    yaxis2 = list(type = "linear", range = list(0, max_display), side = "right", title = "Final Wealth ($) [Linear]", gridcolor = "rgba(240,240,240,1)")
  )
```
