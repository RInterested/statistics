...
<img height = 300  src ="https://user-images.githubusercontent.com/9312897/170094667-a6b9af3f-fca6-4756-8147-f2988b4a7897.gif">
...
The negative of the shape operator is the [**second fundamental form**](https://en.wikipedia.org/wiki/Second_fundamental_form#Physicist's_notation), denoted $\rm{II}$:
...
so it is the negative of the dot product of the shape operator at $P$ evaluated at $\vec v$ and $\vec w.$  

It is therefore more than a change of sign: The shape operator "eats a vector and outputs a vector," whereas the second fundamental form would "eat two vectors and output a real". But in both cases, a matrix representation is possible and routine.
...
and now for the second fundamental form it is necessary to note that the dot product for the entries in the matrix are not $dN$ as the definition would call for, but $N:$
...
The covariant-contravariant form is the result of raising an index by multiplying by the inverse of the metric tensor, which is simply the first fundamental form, i.e. $$\text I^{-1}\text{II}=B^\alpha_\beta = I^{\alpha\gamma}B_{\gamma\beta}.$$ 
