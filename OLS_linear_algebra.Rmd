...
because we are dotting every vector in the $\mathscr C(\mathbf{X})$ with the error vector, and the errors are orthogonal. In other words, the error $\mathbf{e}$ is in the null space of $\mathbf{X}^T$.
...
$\color{blue}{\mathbf{X}^T \mathbf{X\,\hat \beta} = \mathbf{X}^T \mathbf{y}} \text{ NORMAL EQUATIONS}\tag { 3}$
