...
The division polynomial is fixed for a given curve because it represents the intrinsic geometry of that curve. The lines change based on the point. The polynomial changes based on the curve (the coefficients $A$ and $B$). For the elliptic curve $y^2=x^3 +A + B$, when you add a point to itself, the slope $m$ is a function of the coordinates: $m = \frac{3x^2 + A}{2y}$, and the $\psi_3 = 3x^4 + 6x^2 - 1$. This latter equation corresponds to the 3rd division polynomial - $\psi_3(x)$ is the master equation for all points that satisfy $[3]P = \mathcal{O}$. On a donut (the complex torus), the 3rd torsion points form a $3 \times 3$ grid, giving you $9$ points total - in this case for the curve $y^2 = x^3 + x$. There is a general formula for $\psi_3$ for any curve $y^2 = x^3 + Ax + B$:
