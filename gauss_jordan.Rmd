...
These vectors are the solution to the homogeneous system of linear equations, and span the null space of $A$, plus they are linearly independent. Hence they are a $\color{blue}{\text{basis of the null space}}.$ It has a dimension of $3$, corresponding to the number of free variables.
...
A $\color{blue}{\text{basis of the row space}}$ can be found in the **non-zero rows of any echelon form**: $\begin{bmatrix}1&-1&2&3&0\\0&0&1&1&-1\\\end{bmatrix}.$ 

A $\color{blue}{\text{basis of the null space of the transposed (left null space) }N(A^\top)}$ can be obtained by transposing the bottom rows of $E$, corresponding to the zero rows in the echelon form: $\begin{bmatrix}3&1&-5&0\\-4&0&5&1\end{bmatrix}.$
