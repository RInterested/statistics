...
To return to the origin, an even number of steps are needed. The probability to return to origin at a particular step $2n$ is:
...
The initial state is $f_0=0$ and $p_0=0$.
...
and set up the following generating functions from the power series:

$$P(x) = \displaystyle\sum_{n=0}^\infty p_n\,x^n$$

and

$$F(x) = \displaystyle\sum_{n=0}^\infty f_n\,x^n$$

Now we re-write this convolution in this domain:

$$\large P(x)-1=P(x)F(x)\tag 3$$

The number $1$ has to do with the initial conditions.


---

Now, and courtesy of [this post](http://math.stackexchange.com/a/1849538/152225),

$$\displaystyle\sum_{n=0}^\infty\,{2n\choose n}\,x^n=\frac{1}{\sqrt{1-4x}}=(1-4x)^{-1/2}$$
...
$$\color{purple}{\large P(x)= \displaystyle\sum_{n=0}^\infty=\,p_n\,x^n={2n \choose n}\,(pqx)^n=(1-4pqx)^{-1/2}}\tag 4$$
...
$$\color{purple}{\large F(x)=1 - (1-4pqx)^{1/2}}\tag 5$$

---

Next:

$$\sqrt{1-x}=\displaystyle\sum_{m=0}^\infty\frac{-1}{2m-1}\,{2m \choose m}\,\frac{x^m}{4^m}\tag 6$$
...
$f(x)$ has the series expansion:

$$f(x)=1-\sum_{n=1}^\infty \frac{(2n-3)!!}{2^n\,n!}\,x^n$$
...
$$\large F(x)=\displaystyle \sum_{n=0}^\infty f_n x^n= \sum_{n=1}^\infty \frac{1}{2n-1}{2m\choose n}(pq)^n\,x^n$$

Therefore,

$$\large f_n=\frac{1}{2n-1}\,{2n\choose n}(pq)^n$$

A probability generating function for a random variable $X$ taking values in $\{0,\infty\}$ will converge at $x=1$. So at $x=1$ sum of all the $f_n$ is simply the probability that the random walk ever returns to the origin :

$$\large F(1)=\displaystyle \sum_{n=1}^\infty f_n= 1 - \sqrt{1-4pq}=1-|p-q|$$
