...
$\large y_t = c + \color{red}{\rho}\, y_{t-1}+\epsilon_t$
...
**For autoregression assessment we use PACF instead of ACF.**
...
There are a number of significant partial autocorrelation values. The `acf` gives us the expected $1$ for the first value (comparing $t$ to $t$), but the following two values should also be significant. **For moving average assessment we use ACF instead of PACF.**
