...
s <- replicate(1e5, sum(sample(xy, length(x)))) # 1e5 samples of length(x) w/o replac. from combo xy, returning the sum of their elements.
mean_x_samples <- s / length(x)           # 1e5 means from length(x) samples from xy
diff <- sum(xy) - s                       # Difference between combo vec and samples
mean_y_samples <- diff / length(y)        # 1e5 means from length(y) samples from xy
s <- mean_x_samples - mean_y_samples
stat <- mean(x) - mean(y)
(p_value = mean(s > stat))
```
