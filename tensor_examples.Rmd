...
According to [Wikipedia](https://en.wikipedia.org/wiki/Tensor_product#Tensor_product_of_vector_spaces#Tensor_product_of_linear_maps) is the bases vectors are fixed the two tensors $A$ and $B$ result in the tensor product given by the Kronecker multiplication: 
...
\end{bmatrix}
$$
I presume that $T$ would have two upper and two lower indices, as in $T^{ij}_{\phantom{ij}kl}$. 

Now I should be able to contract $i$ and $k,$ equivalently of taking the trace of the $A$ matrix and multiplying it by $B$, i.e. $\mathop{Tr}(A)B$. This would correspond to $T^{ij}_{\phantom{ij}il}$. 
...
This makes perfect sense. Thank you! As a quick follow-up question, what would be the mechanics if instead I wanted to contract $j$ and $l$, or $i$ and $l$ in $T^{ij}_{\phantom{ij}kl}$? (Unless it is worth a different question, or can't be answered in a quick couple of lines).
...
As far as I know, all possible contractions with the selection of pairs of indices are the following:<br/>
<br/>- 1st covariant index and 1st contravariant index.<br/>
...
$\begin{align}a_{11}b_{11} + a_{22}b_{11} &=-1 + (-3) = -4\\
...
> I have to qualify my above comment that it is always traces of submatrices. That holds if you contract over the two indices of $A$ or over the two indices of $B$, but if you contract over, say, $\color{red}i$ and $\color{yellow}l$ - in $T^{{\color{red}i}j}_{\phantom{ij}k{\color{yellow}l}}$ - then you would have a matrix product.

---

##### Question:

If I want to contract (as in one of the comments to the prior post) $i$ and $l$, what would be the mechanics (algorithm) it if all I have is the final Kronecker block matrix above? (no $A$ or $B$ explicitly given).
...
$$\begin{matrix}
j = 1, k = 1: & T^{i1}_{\phantom{i1.}1i} = a_{1,1}b_{1,1} + a_{2,1}b_{1,2}\\
j = 1, k = 2: & T^{i1}_{\phantom{i1.}2i} = a_{1,1}b_{2,1} + a_{2,1}b_{2,2} \\
j = 2, k = 1: & T^{i2}_{\phantom{i2.}1i} = a_{1,2}b_{1,1} + a_{2,2}b_{1,2} \\
j = 2, k = 2: & T^{i2}_{\phantom{i2.}2i} = a_{1,2}b_{2,1} + a_{2,2}b_{2,2}
\end{matrix}
$$
or
$$
\begin{matrix}
i = l = 1 & i = l = 2\\
a_{{\color{red}1}j} b_{k{\color{red}1}} & a_{{\color{red}2}j} b_{k{\color{red}2}}\\
--- & ---\\
...
b_{12} & b_{22}
\end{bmatrix}
=\begin{bmatrix} a_{1,1}b_{1,1} + a_{2,1}b_{1,2} & a_{1,1}b_{2,1} + a_{2,1}b_{2,2} \\ a_{1,2}b_{1,1} + a_{2,2}b_{1,2} & a_{1,2}b_{2,1} + a_{2,2}b_{2,2} \end{bmatrix}$$
...
The result of contracting over the first and fourth indices can be written as a square $2\times2$ matrix composed of four traces of $2\times2$ submatrices like in the previous question; for example, the $(j=1,k=1)$ component of the contraction is the trace of the submatrix consisting of elements in rows $1$ and $3$ and in columns $1$ and $2:$
...
Right! The tidy packaging is also constraining - its order in rows and cols functions as a proxy for the basis vectors, but typically, this would be part of the tensor in a less constraining notation, such as $Y^{jk}e_j⊗e_k$. 

At the end of these couple of questions / answers, it seems that there is no difference in the mechanics between contracting a tensor or tensor multiplying (for instance a tensor ⊗ (eating) a vector).
