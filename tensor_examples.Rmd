...
Now I should be able to contract $i$ and $k,$ equivalently of taking the trace of the $A^i{}_{k}$ matrix and multiplying it by $B$, i.e. $\mathop{Tr}(A)B$. This would correspond to $T^{ij}{}_{il}$. 
...
$$\begin{bmatrix}
a_{11}b_{11} + a_{22}b_{11} & a_{11}b_{12} + a_{22}b_{12} \\
a_{11}b_{21} + a_{22}b_{21} & a_{11}b_{22} + a_{22}b_{22}
\end{bmatrix} =
(a_{11} + a_{22}) 
\begin{bmatrix}
b_{11} & b_{12} \\
...
are the traces in your answer when applied to $Y$ and $Z$, and correspond to $\text{Tr}(Y)Z=4 \begin{pmatrix}-1&0\\1&2\end{pmatrix}$
...
If I want to contract $i$ and $l$, what would be the mechanics (algorithm) it if all I have is the final Kronecker block matrix above? (no $A$ or $B$ explicitly given).
