...
This is [equivalent to the z test statistic](http://rinterested.github.io/statistics/one_sample_test.html) for sample means, and does follow a Z distribution in large samples.

Notice that since we are testing under the $H_o$ how likely it is to get the $p$ we obtained we use the population proportion $p_o$ to calculate the standard error, as opposed to the sample proportion we would use to build the confidence interval:
