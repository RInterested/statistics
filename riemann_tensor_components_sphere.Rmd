...
This teddy bear of an equation is directly derived from the fact that we are using the [Levy-Civita connection](https://en.wikipedia.org/wiki/Levi-Civita_connection), which is torsion-free, and metric compatible:
...
$$\Gamma^m_{jk}=\frac 1 2 g^{im}(\partial_k g_{ij}+ \partial_j g_{ki }- \partial_i g_{jk})$$

So for instance, in the sphere and for $\lambda=1=\theta, \mu=2=\phi,\nu=2=\phi$

$$\Gamma^{\theta}_{\mu\nu}=\Gamma^{\theta}_{\phi\phi}=\Gamma^1{}_{22}=\sum_{σ=1}^n g^{λσ} (∂_μ g_{σν} + ∂_ν g_{σμ} - ∂_σ g_{μν})$$
