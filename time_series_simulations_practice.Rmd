...
set.seed(2024)
n = 1000
c = arima.sim(n=n,list(ar=c(0.6)), innov=rnorm(n))
...
set.seed(2024)
n = 1000
AR4.sim <- function(n, ar1, ar2, ar3, ar4, sd)
{
  Xt = c(0,0,0,0)
  for (i in 4:n)
  {
    Xt[i+1] = ar1*Xt[i] + ar2*Xt[i-1] + ar3*Xt[i-2] + ar4*Xt[i-3] + rnorm(1,mean=0,sd=sd)
  }
  Xt
}
a = AR4.sim(n, 0.15, 0.3, 0.5, 0.8, 2)

r1 <- ggAcf(a, lag.max = 50) + labs(title = "ACF AR(4)")
r2 <- ggPacf(a, lag.max = 50) + labs(title = "PACF AR(4)")
plot_grid(r1, r2, ncol = 2)
```
