...
The original dataset is altered using the defined functions `chg` and `lag` that calculate along columns the differences between one measurement and the preceding (`chg`), and slide the values one step along the time line: 
...
showcoef <- function(formula, controls= (~.), data=ds){
  round(coef(summary(lm(update(formula, controls),  data=data)))[,c(1,3,4)],3)
} 
