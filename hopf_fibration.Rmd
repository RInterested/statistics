---
title: 'Hopf Fibration and gauge theory intuition'
output: 
  html_document:
    theme: readable
    includes:
...
The fact that every pair of fibers in the Hopf Fibration is linked exactly once tells us something profound: $S^{3}$ is not just a simple "box" made of a sphere and a circle. In topology, we say that the linking of the fibers proves that the 3-sphere is a nontrivial fiber bundle. Here is what that "linking" reveals about the nature of the space: $S^{3}$ is not $S^{2} \times S^{1}$. If you simply took a 2D sphere ($S^{2}$) and "multiplied" it by a circle ($S^{1}$), you would get a space where the circles just sit next to each other like parallel lines on a sheet of paper. In a trivial space if you picked two points on the sphere and looked at their fibers, they would be like two separate rings sitting side-by-side. They would not be linked. In $S^{3}$ the linking tells us there is a "twist" in the very fabric of the dimensions. You cannot separate the circle fibers without cutting them. This "twist" is the reason why $S^{3}$ is a "single piece" topologically, rather than a simple product of two smaller shapes. The linking number of the fibers is a topological "ID card" called the Hopf Invariant. Because any two fibers have a linking number of 1, the Hopf Map is said to have an invariant of 1. This is the mathematical way of saying that the mapping from $S^{3}$ to $S^{2}$ is "essential" — it cannot be shrunk down to a single point without tearing the space. If the fibers weren't linked, you could essentially "unroll" the 3-sphere into a flat 3D space. The linking is what "locks" the $S^{3}$ into its closed, spherical shape. The Hairy Ball Theorem, which says you can't comb a 2D sphere ($S^{2}$) without a cowlick is bypassed by the linking in $S^{3}$. Because the fibers are linked and "swirl" around each other, they create a smooth flow that never has to stop or "pinch" at a pole. This makes $S^{3}$ a parallelizable manifold. It is one of the only spheres in existence (along with $S^{1}$ and $S^{7}$) where you can have a perfectly smooth, non-vanishing field of vectors everywhere. The linking is the geometric "secret" that allows the vectors to pass each other without ever crashing into a singularity. The linking explains why the "Gauge" angle $\psi$ is so important. As you move around a loop on the base sphere $S^{2}$, the fibers don't just stay parallel; they rotate relative to one another. By the time you come back to your starting point on the base sphere, the "linkage" has forced the fiber to have gone through a full rotation. This is exactly how Magnetic Monopoles work in physics. The linking of the Hopf fibers is the topological reason why a magnetic charge must be quantized — it has to "link" a whole number of times to stay mathematically consistent.

The linking of the Hopf fibers reveals that $S^3$ has a shape that is compact, closed, and "twisted." It tells us that $S^3$ is not just a larger version of a normal sphere, but a structure where every "straight" path eventually loops back and interlocks with every other path. In 3D, if you rotate an object $360^\circ$, it looks the same. But in the shape of $S^3$, you have to go around twice ($720^\circ$) to truly return to your starting state. This is because the fibers are linked; one full trip around a fiber ($S^1$) only gets you halfway around the "twist" of the whole sphere. This is why $S^3$ is the shape used to represent Spin-1/2 particles (like electrons) in physics.
...
<img width = 400  src = "villarceau_circles.gif">
...
$$(z_0, z_1) = e^{i\color{red}\psi} \cdot \frac{1}{\sqrt{1+|q|^2}}(q, 1)$$
 
As $\psi$ ranges from $0$ to $2\pi$, you trace out the $S^1$ circle in $4D$. And thus is born the $U(1)$ symmetry. Global Symmetry: If you shift $\psi$ by the same amount everywhere in the universe, the Lagrangian remains unchanged. This leads (via Noether's Theorem) to the conservation of charge. If you want to change $\psi$ differently at every point in space—meaning you move along different fibers in the Hopf bundle independently—the Lagrangian would normally break. To "fix" it, you must introduce a gauge field (like the Photon). The **gauge field** and the **connection (covariant derivative)** are essentially two sides of the same coin. In physics, we call it a gauge field; in differential geometry, we call it a connection on a fiber bundle. 

In the Hopf Fibration as a "Field" the Base Sphere ($S^2$) represents the "Physical Field Strengths" (the observables, like the Magnetic Field $\mathbf{B}$).The Total Space ($S^3$) represents the "Vector Potential" (the gauge field $\mathbf{A}$).The reason the Hopf Map exists is that the vector potential $\mathbf{A}$ has more information than the magnetic field $\mathbf{B}$. That "extra" information is precisely the phase $\psi$.

In the context of the Hopf Fibration, the Associated Bundle is how we move from the "pure geometry" of circles to the "physical reality" of fields and particles. While the Hopf Fibration itself is a Principal Bundle (where the fiber is the symmetry group $U(1)$ itself), an Associated Bundle is what you get when you let that $U(1)$ symmetry act on something else—specifically, a complex line. The most important associated bundle in this setting is the Complex Line Bundle (often denoted as $L$). Instead of having a circle ($S^1$) sitting over every point of the base sphere $S^2$, you attach an entire complex plane ($\mathbb{C}$) to every point. The "direction" or "phase" within that complex plane is governed by the Hopf Fibration. In physics, the sections of this bundle (the way you pick one complex number for every point on the sphere) are the Wavefunctions $\psi(x)$ of a quantum particle. To create this bundle, you take the 3-sphere $S^3$ and a complex plane $\mathbb{C}$, and you "glue" them together using the gauge symmetry $\psi$. Mathematically, you define an equivalence relation: $$(p, v) \sim (p \cdot e^{i\psi}, e^{-i\psi} v)$$ Where $p$ is a point in $S^3$ and $v$ is a complex number in $\mathbb{C}$. If you rotate your position along the fiber in $S^3$, you must rotate the value in the complex plane by the opposite amount to compensate. This "locks" the complex plane to the Hopf fiber. Depending on how "fast" you rotate the complex plane relative to the Hopf fiber, you get different associated bundles, categorized by an integer $k$ (the Chern Number): $k=1$ (The Standard Line Bundle): This represents a particle with a single unit of electric charge interacting with a Dirac Magnetic Monopole. $k=1/2$ (The Spinor Bundle): This is the bundle of Spinors. The linking of the Hopf fibers is what allows spinors to exist; it explains why they must rotate $720^\circ$ to return to their original state.Higher $k$: Represents particles with higher charges or higher "winding numbers."

Imagine the Hopf bundle over the sphere. At every point $p$ on the base sphere $S_2$, you have a circle fiber in 4 dimensions. If you move from point $A$ to point $B$, you want to compare the phase $\psi_A$ to $\psi_B$. But if $\psi$ is allowed to change "randomly" at every point (Local Gauge Symmetry), you have no way to know if a change in the field is a "real" physical change or just a change in your local choice of $\psi$.The standard derivative $\partial_\mu$ fails here because it compares values at two points as if the space were flat, ignoring the "twist" of the fibers. To compare phases at different points, you need a "rule" for how to stay "level" as you move from one fiber to the next. This rule is the connection. In physics, this connection is the Vector Potential $A_\mu$. It defines what "horizontal" movement looks like across the fibers. When you introduce $A_\mu$, you are essentially saying: "I know the phase is changing locally, so I will introduce this field to keep track of that change and subtract it out." The Covariant Derivative is the tool that uses the connection to perform "fair" calculus. It replaces the standard derivative $$D_\mu = \partial_\mu - iqA_\mu$$ 

$\partial_\mu$ measures how the field changes.$-iqA_\mu$ corrects for how the "gauge" (the fiber orientation) is changing. By using $D_\mu$, the **Lagrangian** becomes "blind" to the local choice of $\psi$. The "breaking" is fixed because the gauge field $A_\mu$ absorbs the "mess" created by the local phase changes.

We defined $q$ as the ratio of the two complex coordinates in $S^3$: 

$$q = \frac{z_0}{z_1}$$ 

If we want to work backward to find $(z_0, z_1)$ using only $q$, we can factor out $z_1$ from the vector: 

$$(z_0, z_1) = z_1 \cdot \left( \frac{z_0}{z_1}, \frac{z_1}{z_1} \right) = z_1 \cdot (q, 1)$$ 

The "1" is simply the result of dividing $z_1$ by itself. It acts as the reference unit for the ratio. The vector $(q, 1)$ points in the correct "direction" in complex space, but it isn't on the 3-sphere yet. To be on $S^3$, the total magnitude squared must be exactly 1: $|z_0|^2 + |z_1|^2 = 1$. If we check the magnitude of our raw vector $(q, 1)$, we get: 

$$\sqrt{|q|^2 + |1|^2} = \sqrt{|q|^2 + 1}$$ 
To force this vector to land exactly on the surface of the sphere, we must divide by that magnitude. That is where the normalization factor comes from:$$\text{Unit Vector} = \frac{1}{\sqrt{|q|^2 + 1}}(q, 1)$$

Every complex number can be written in polar form as $z = R e^{i\psi}$, where $R$ is the magnitude and $e^{i\psi}$ is the phase. So: 

$$z_1 = |z_1| e^{i\psi}$$ 
When we substitute this into the vector $(q, 1)$, we get 

$$(z_0, z_1) = |z_1| e^{i\psi} (q, 1)$$ 
Solving for the Magnitude $|z_1|$, we know that for any point in $S^3$, the sum of the squares of the magnitudes must be 1: $|z_0|^2 + |z_1|^2 = 1.$ Since $z_0 = q z_1$, we can substitute that in: $|q z_1|^2 + |z_1|^2 = 1$, and $|q|^2 |z_1|^2 + |z_1|^2 = 1$. Now, factor out the $|z_1|^2$: $|z_1|^2 ( |q|^2 + 1 ) = 1$, and $|z_1|^2 = \frac{1}{1 + |q|^2}$. Taking the square root gives us the magnitude of $z_1$: $|z_1| = \frac{1}{\sqrt{1 + |q|^2}}$. Now we substitute that magnitude back into our polar form of $z_1$: 

$$z_1 = \underbrace{\frac{1}{\sqrt{1 + |q|^2}}}_{\text{Magnitude}} \cdot \underbrace{e^{i\psi}}_{\text{Phase}}$$ 

So the equation:$(z_0, z_1) = z_1 (q, 1)$ becomes: 

$$(z_0, z_1) = \left( \frac{1}{\sqrt{1 + |q|^2}} e^{i\psi} \right) (q, 1)$$
...
The core of the Hopf Fibration is the idea that we can "ignore" the global phase. We say that two points $(z_0, z_1)$ and $(w_0, w_1)$ are "equivalent" if one is just the other multiplied by a complex number of magnitude 1 ($e^{i\psi}$).

$$(z_0, z_1) \sim (e^{i\psi}z_0, e^{i\psi}z_1)$$
...
#### From coordinates in $\mathbb R^4$ to $\mathbb R^3$:
...
$$\begin{cases} X = 2 \text{Re}(z_0 \bar{z}_1) \\ Y = 2 \text{Im}(z_0 \bar{z}_1) \\ Z = |z_0|^2 - |z_1|^2 \end{cases}$$
These take a point in $S^3$ (the 3-sphere) and tell you where it "lands" on the $S^2$ (the base sphere). A point on a unit sphere in 3D space $(X^2 + Y^2 + Z^2 = 1)$. It collapses the entire circle fiber down to a single point. It is a "compression" of dimensions.

A point in $S^3$ is a pair of complex numbers $(z_0, z_1)$. The Hopf map's job is to extract the relative relationship between them while throwing away their shared rotation (the gauge symmetry).

> The Z-Axis: The "Balance Scale" 

$$Z = |z_0|^2 - |z_1|^2$$ Think of $z_0$ and $z_1$ as two reservoirs of "magnitude." Since $|z_0|^2 + |z_1|^2 = 1$, this formula measures the difference in how much "weight" is in each. 

If all the weight is in $z_0$: $Z = 1 - 0 = 1$ (The North Pole).
If all the weight is in $z_1$: $Z = 0 - 1 = -1$ (The South Pole).
If they are perfectly balanced: $Z = 0.5 - 0.5 = 0$ (The Equator).

Intuition: $Z$ tells you the "altitude" on the base sphere based solely on the ratio of the sizes of the two complex numbers.

>The X and Y Axes: 

The "Phase Comparator": $$X + iY = 2z_0 \bar{z}_1$$ This is the most ingenious part. By multiplying $z_0$ by the conjugate of $z_1$, you are essentially calculating the difference in their angles. In complex math, when you multiply $A$ by $\bar{B}$:The magnitudes multiply.The angles subtract.Why this matters: If you rotate both $z_0$ and $z_1$ by the same "gauge" angle $\psi$ (the fiber movement), the subtraction in $z_0 \bar{z}_1$ cancels that $\psi$ out perfectly. What remains is the relative phase ($\phi$) between them.

$X$ (Real part): Measures how much the two complex numbers are "in sync."

$Y$ (Imaginary part): Measures how much they are "out of sync" (quadrature).

Intuition: $X$ and $Y$ tell you the "longitude" on the base sphere by looking at how the two complex numbers are rotated relative to each other.
