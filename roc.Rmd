...
Notice the values in the $18$-th row of the `ROC` printout - they are the values printed on plot above as `lr.eta` values:

```{r}
rock$res[18,]
```    

The ***maximized sum threshold or MST*** or the [**probability** of the outcome](http://rinterested.github.io/statistics/logistic_regression.html) is the `eta`:
...
```{r}
roc(aSAH$outcome, aSAH$s100b)$threshold
```
...
Let's add the predicted values, which are predicted PROBABILITIES:
...
* For cut[1] = 0, there will be zero outcomes predicted as "Poor" - $\text{sens} = \frac{1\text{ TP}}{1\text{ "Poor"}}$, $\text{spec} = \frac{0\text{TN}}{4\text{"Poor"}}$
* For cut[2] = 0.02880979, $\text{sens} = \frac{1\text{TP}}{1\text{"Poor"}}, $\text{spec} = \frac{1\text{TN}}{1\text{"Poor"}}
