...
If you flip a fair coin $n$ times (where heads = $+1$ and tails = $-1$), your position after $n$ steps is $S_n$. What happens as $n$ grows massive?
...
The **Central Limit Theorem (CLT)**: Tells you the distributional behavior. It says that $S_n$ grows roughly on the scale of $\sqrt{n}$, and if you divide $S_n$ by $\sqrt{n}$, you get a standard normal distribution.
...
This second result means that if you look at a Brownian path right at the starting needle-point of $t=0$, it is oscillating up and down with infinite frequency, perfectly bounded by an local envelope scaled by $\sqrt{2t \log(\log(1/t))}$.


### RANDOM WALK VARIANCE:

If you flip a fair coin at every step in time, your steps are independent. Because of that independence, variance adds up linearly. At time step $t$, the variance of your position is exactly:
...
As time moves forward, the system is constantly injecting new randomness into the environment. The "space" of where you could possibly be is expanding continuously. To turn this variance into a physical distance or a spatial boundary (with units), we take its square root to get the standard deviation: $\sigma\sqrt{t}.$ This is the standard scale of a random walk. At any single snapshot in time, your path has a standard width proportional to $\sqrt{t}$.

Now, suppose you track one single path over a timeline that stretches to infinity ($t \to \infty$). Because the variance grows linearly forever, this single path has an infinite amount of time to wander. More importantly, it has infinite opportunities to experience rare, highly improbable streaks — like flipping heads $20$ times in a row. Because of these infinite attempts, a single path will routinely break right out of its standard $\sqrt{t}$ boundary. If you try to contain a random walk using only its standard deviation, the walk will escape your bound infinitely many times as $t \to \infty$. A simple square root cannot hold it. 
