...
$$f(\theta|X=1)=\frac{\theta^1(1-\theta)^0I_{(0\leq\theta\leq1)}}{\int_{-\infty}^\infty \theta^1(1-\theta)^0I_{(0\leq\theta\leq1)}d\theta}=\frac{\theta^1I_{(0\leq\theta\leq1)}}{\int_0^1 \theta^1I_{(0\leq\theta\leq1)}d\theta}=2\thetaI_{(0\leq\theta\leq1)}\tag 1$$
...
Therefore the likelihood function is $\large f(X|\lambda) = \frac{\lambda^{\sum x_i}\,e^{-n\lambda}}{\displaystyle \prod_{i=1}^n x_i!}$
