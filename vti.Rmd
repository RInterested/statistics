...
VTI$Date <- as.Date(VTI$Date)
rownames(VTI) <- VTI$Date
chartSeries(VTI[,-1], type="auto", theme=chartTheme('white'))
```
...
```{r}
rownames(VTI)[VTI$InterDay == min(VTI$InterDay)]
rownames(VTI)[VTI$InterDay == max(VTI$InterDay)]
```
...
plot(VTI$Date, VTI$InterDay < -4, pch=19, col=2, type='h', xaxt="n",
     xlab="Year", ylab="Days with > - 4% change", 
     cex.axis=.5, cex.main=.8, cex.lab =.5,las=2, 
     main  = "Clustering of big drop days")
axis(side=1, at=as.Date(paste0(2001:2017, "-01-01")), labels = 2001:2017, las=2, cex.axis=.7)
```

... and the wins...

```{r, fig.height=5, fig.width=7}
plot(VTI$Date, VTI$InterDay > 4, pch=19, col=3, type='h', xaxt="n",
     xlab="Year", ylab="Days with > 4% change", 
     cex.axis=.5, cex.main=.8, cex.lab =.5,las=2, 
