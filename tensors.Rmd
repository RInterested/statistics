...
> KEY POINT: The gradient vector is in the dual space, taking in a "regular" vector and producing a scalar. In the case of the contravariant vector, a vector in a coordinate frame was transformed into also a vector a different frame.
...
An **ENDOMORPHISM** is a linear map of $V$ onto itself. Hence, $End(V) := Hom(V,V).$

An **AUTOMORPHISM** is an **invertible** linear map of $V$ on itself. Hence, $Aut(V) := \{f: V {overset}{sim}\rightarrow V | \text{invertible}\}$

Automorphisms are a subset $Aut(V) \subset End(V)$ of endomorphisms.

---

We can define the set of **ALL LINEAR MAPS** from $V \rightarrow W$ as $Hom(V,W):= \{f:V \overset{\sim}\rightarrow W\}$ with the $\sim$ denoting *linear* map.

Is this set of all linear maps a vector space? Yes, by defining,

$$\boxed{\color{blue}{+}}: Hom(V, W) \times Hom(V,W) \rightarrow Hom(V,W)$$ by taking the pair of linear functions and mapping it to
...
$$T: \underset{p}{\underbrace{V^*\times \cdots \times V^*}}\times \underset{q}{\underbrace{V\times\times \cdots  \times V}} \overset{\sim}\rightarrow K$$

The **$(p,q)$ TENSOR SPACE** is defined as a set:

$$\begin{align}T^p_q\,V &= \underset{p}{\underbrace{V\otimes\cdots\otimes V}} \otimes \underset{q}{\underbrace{V^*\otimes\cdots\otimes V^*}}:=\{T\, |\, T\, \text{ is a (p,q) tensor}\}\tag1\\[3ex]&=\{T: \underset{p}{\underbrace{V^*\times \cdots \times V^*}}\times \underset{q}{\underbrace{V\times \cdots \times V}} \overset{\sim}\rightarrow K\}\end{align}\tag2$$ 

is the set of all tensors where $T$ is (p,q),

equipped this with pointwise addition and s-multiplication:

E.g. if we have two $(1,1)$ tensors, $T$ and $(S)$,

$$\large (T + S)(\underset{\sigma\in V^*}{\underbrace{\sigma}}\, \underset{v\in V}{\underbrace{v}}):= T(\sigma, v) \underset{\text{operation defined in K}}{\underbrace{+}} S(\sigma, v)$$

and

$$(\lambda \cdot T)(\sigma, v):= \sigma \underset{\text{def. in k}}{\underbrace{\cdot}}T(\sigma, v).$$


---

We can say that the $p$ elements $V^*\times V^*\times\cdots$ in Eq. 2 are linear functionals waiting for a vector $V$ to produce real numbers that are later multiplied. The $q$ $V$ elements in $V\times V\times\cdots$ are vectors in $V$ "waiting" for a functional to be mapped into $K$? Since we are defining a set, it can be any functional.

---

Notice that $\left(T^p_q, +, \cdot \right)$ is a vector space.
