...
final <- as.table(Antacid)
final[] <- paste(as.table(Antacid),paste0("(",as.table(expect),")"))
final
```
...
It is also possible to see the term **GOF ("goodness-of-fit")** applied exclusively to testing of whether some observed counts *conform* to a theoretical proportion. We assess whether observed counts fit into the distribution of hypthesized counts or theoretical values.

As an example, we can test how good is [R] at generating random uniform numbers by generating $\small 1 \,million$ draws:
...
colnames(tab) <-c("[0,0.25)", "[0.25,0.5)", 
                        "[0.5,0.75)", "[0.75,1)")
...
Now we can run the test with degress of freedom corresponding to the cells minus 1 (in this case $\small df=3$). The name I gave to the table presenting the counts and expected frequencies is `tableau`:
...
The idea is to run a GoF (goodness-of-fit) test to see if the data conforms to the expected frequencies. And we would do this with a χ2 test. The degrees of freedom are the number of cells minus 1.
...
We needed to "discretize the data" to run the GOF $\chi^2$, but we can do the same thing without discretization with the Kolmogorov-Smirnov test.
