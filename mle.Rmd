...
$$\mathcal L(p;x) = f_X(x;p)=\prod_{i=1}^{n}f_X(x_i;p)=\prod_{i=1}^{n} p^x(1−p)^{1−x}$$

Differentiating the $\log \mathcal L(p ; x)$ with respect to $p$ and setting the derivative to zero shows that this function achieves a maximum at $\hat p=\sum_{i=1}^n x_i/n$. Since  $\sum_{i=1}^n x_i/n$ is the total number of successes observed in the $n$ trials, $\hat p$ is the observed proportion of successes in the $n$ trials. We often call $\hat p$ the sample proportion to distinguish it from $p$ , the “true” or “population” proportion. 
