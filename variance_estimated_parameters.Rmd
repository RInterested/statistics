...
From [this video](https://youtu.be/jyBtfhQsf44?list=PLVhS9-aIl4U-Guw_wvCHMhF2unUGVyWEu):

We know that $\hat \beta= \left(X^\top X \right)^{-1} X^\top y.$ But we want to calcualte the variance:

\begin{align}\text{var}[\hat \beta] &= \text{var}\left[\left(X^\top X \right)^{-1} X^\top y\right]\\\quad
...
and since $X\beta$ is fixed, $\text{var}(y)= \text{var}\left(X\beta+\varepsilon\right)=0+\text{var}(\epsilon)=\sigma^2 \bf I$, since $\varepsilon \sim N(0, \sigma^2 \bf I).$
