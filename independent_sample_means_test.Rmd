...
where $n_i$ is the sample size of population $i$ and $s^2_i$ is $\Large \frac{1}{n_i-1} \sum_{j=1}^{n_i} \left(y_j - \bar y_j \right)^2$ . Use of $(n_i-1)$ weighting factors instead of $n_i$ comes from Bessel's correction.

For two samples the $S_p$ will therefore be:

$\Large S_p\,=\,\sqrt{\frac{(n_x - 1)\,S_x^2\,+\,(n_y - 1)\,S_y^2}{n_x + n_y -2}}$

<br>

**Example:**

Example: We want to look into the effects of Dexmethylphenidate on weight in children with ADHD, and we randomized `n1 = 15` children to treatment, and `n2 = 15` to placebo, measuring weight before the beginning of the treatment and again 2 months later. The treated group dropped an average of `delta1 = -2.4 lbs.` (with a `SD1 = 0.4 lbs.`), while the placebo group did also drop, but only `delta2 = -0.9 lbs.` (with a `SD2 = 0.8 lbs.`). Is the difference significant (two-tailed test)?
...
In our case: $sp^2 =\frac{(n1-1) * SD_1^2 \,+\, (n2-2)^ * SD_2^2}{n1 + n2 -2} = \frac{1}{2} (SD_1^2+ SD_2^2)$. 

The SE will be calculated as $SE = \sqrt {sp^2} * \sqrt {\frac{1}{n1}+\frac{1}{n2}}$. 
...
$(\bar X - \bar Y) \pm qt(0.975, n_x + n_y - 1) \,S_p\, \sqrt{\frac{1}{n_x}+\frac{1}{n_y}}$

And for larger samples:

$(\bar X - \bar Y) \pm qnorm(0.975)\,S_p \,\sqrt{\frac{1}{n_x}+\frac{1}{n_y}}$
