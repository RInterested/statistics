...
In standard analysis, a geometric series $\sum r^n$ only converges if $|r| < 1$. But the p-adics redefine distance: by declaring that a number is small if it is divisible by a high power of $p$ we force the geometric series to converge for ratios like $r=3$, $r=5$, or $r=p$. From an algebraic perspective, the rationale is that the p-adic integers $\mathbb{Z}_p$ are the completion of the integers $\mathbb{Z}$ with respect to this p-adic metric. 
