...
The reason they "coincide" for integers is that both methods — the quotient-based one and the remainder-based one— are actually doing the exact same thing to the number's structure, just from opposite directions. When you convert an integer like $450$ to base $5$, you are essentially "unpacking" the number.

- Quotient Method (Base $5$): 

  - You ask, "How many $5^3$s ($125$s) fit in here?" There are $3$ (totaling $375$), leaving $75$. 

  - Then, "How many $5^2$s ($25$s) fit in $75$?" There are $3$, leaving $0$.
  
- Remainder Method ($5$-adic): 

  - You ask, "What is the remainder when I divide $450$ by $5$?" It is $0$. That is your $5^0$ digit. 
  
  - You divide the rest by $5$ to get $90$. 
  
  - Then, "What is the remainder of $90$ divided by $5$?" It is $0$. That is your $5^1$ digit.
  
Because an integer is finite, you eventually run out of "stuff" to divide or subtract. Whether you start from the "big" end (powers of $125$) or the "small" end (powers of $1$), you are identifying the same coefficients for the polynomial:

$$450 = (\mathbf{3} \cdot 5^3) + (\mathbf{3} \cdot 5^2) + (\mathbf{0} \cdot 5^1) + (\mathbf{0} \cdot 5^0)$$
The digits are the same because $450$ has a unique representation as a sum of powers of $5$.
