...
Transcendental numbers also turn into p-adic integers... Well, sometimes! Here is a mind-boggling realization: To move a number from $\mathbb{R}$ to $\mathbb{Q}_p$, we need a bridge: usually a sequence of rational numbers. For instance in $\mathbb{R}$, $e = \displaystyle \sum_{n=0}^\infty \frac1 {n!}$. In $\mathbb{Q}_p$, the terms $1/n!$ have $p$ in the denominator more and more often as $n$ grows. This means the terms get larger pically $(|1/n!|_p \to \infty)$. Therefore the series explodes. Similarly $\pi$ is defined by the geometry of Euclidean space. Since pic space is non-Euclidean and ultra-metric (every triangle is isosceles), the physical definition of $\pi$ has no meaning there.

Even though Ostrowski's Theorem states that the only ways to fill in the gaps of the rational numbers $\mathbb{Q}$ are through the familiar real numbers $\mathbb{R}$ or the pic numbers $\mathbb{Q}_p$ for some prime $p,$ we can't express either $\pi$ or $e$ as p-adics. Completion isn't about collecting every possible number that exists; it's about filling in the holes for a specific way of measuring distance. In the reals we use the distance $|x - y|:$ We see that the sequence $3, 3.1, 3.14, 3.141...$ is getting closer and closer together. Since the "limit" isn't a rational number, we call that hole $\pi$ and add it to the set. In the p-adics, we use the distance $|x - y|_p$. In this metric, the sequence $3, 3.1, 3.14...$ doesn't get closer together at all. In fact, the terms stay far apart. Therefore, there is no "hole" at $\pi$ in the pic world because the sequence never tries to go there. The pics are complete because every sequence that should converge (according to pic distance) does converge to a pic number.

Just as in the real world, many Cauchy sequences of rational numbers do not converge to a rational number. The set of pic numbers $\mathbb{Q}_p$ is defined as the completion of $\mathbb{Q}$ with respect to the pic metric. 

The "geometry" of a pic circle is nothing like the one you draw on a piece of paper. Here is a Gemini depiction of a circle as the folders at the same depth inside a Windows Explorer system (explained later on):
...
In the pic world, trigonometry exists through power series, but the lack of $\pi$ means the functions behave in ways that would baffle a high school geometry student. Here is how we compute "circular" things in $\mathbb{Q}_p$ without $\pi$:
...
These series converge in $\mathbb{Q}_p$ as long as $|x|_p$ is small enough (specifically $|x|_p < p^{-1/(p-1)}$). Inside this "disk of convergence," these functions work perfectly. You can even prove the pic version of the Pythagorean identity $\sin^2(x) + \cos^2(x) = 1$.

In the real numbers, $\sin(x)$ is periodic because of $\pi$. You go around the circle $2\pi$ radians and end up back where you started. In the pics, there is no $\pi$ because there is no periodicity. In $\mathbb{R}$, $\sin(x) = 0$ at $0, \pi, 2\pi, \dots.$ In $\mathbb{Q}_p$, the only solution to $\sin(x) = 0$ is $x = 0$. Because the functions aren't periodic, they don't "wrap around." A pic "line" doesn't form a closed loop in the way a real line wraps into a circle. If you define a circle as the set of points where $|x|_p = r$, the shape is strange: a set of disjoint disks. Because of the ultrametric inequality, if two pic "circles" overlap at all, one is contained entirely within the other. A pic circle isn't a smooth continuous line; it's more like a fractal dust (a Cantor set).

Since we don't have $\pi$ to measure angles, we use roots of unity. In the complex numbers, the roots of $x^n - 1 = 0$ are points on the unit circle separated by angles of $2\pi/n$. In $\mathbb{Q}_p$, we can solve $x^n - 1 = 0$ for certain values of $n$ (specifically where $n$ divides $p-1$ using Hensel's Lemma). These roots of unity are the pic way of "dividing the circle." You aren't moving a distance of $\pi/4$ along an arc; you are jumping between discrete algebraic points that satisfy a circular equation.
...
While the imaginary number $i = \sqrt{-1}$ cannot exist in the real number system because a positive times a positive is positive and a negative times a negative is also positive, it does exist in the $5$-adic number system. The reason it only exists for some $p$ is based on modular arithmetic. A square root of $-1$ (a number $x$ such that $x^2 = -1$) exists in the pic integers $\mathbb{Z}_p$ if and only if the equation $x^2 \equiv -1 \pmod p$ has a solution. This condition is only met when $p$ is a prime number that is congruent to $1$ modulo $4$. For $p=5$ the equation $x^2 \equiv -1 \pmod 5$ is equivalent to $x^2 \equiv 4 \pmod 5$. This has two solutions: $x=2$ and $x=3$, as $2^2 = 4 \equiv 4 \pmod 5$ and $3^2 = 9 \equiv 4 \pmod 5$. Because these solutions exist, they can be lifted to form two unique $5$-adic integers that are the square roots of $-1$.
...
While the pic integers $\mathbb{Z}_p$ consist of power series starting at $i=0$, the full field of pic numbers, denoted by $\mathbb{Q}_p$, allows for a finitary fractional part: the "fractional" part (the negative powers) must be finite. If we allowed an infinite number of negative powers, the series would not converge under the pic metric. In pic analysis, $p^n$ gets smaller as $n$ gets larger. Therefore, $p^{-n}$ gets larger as $n$ gets larger. An infinite sum of increasingly huge terms cannot converge.
...
$\mathbb{Q}_p$ can be viewed as the field of fractions of $\mathbb{Z}_p$. There is resemblance to Laurent series, in which the variable is $x$ or $(z - z_0)$, while in pics, the prime $p$ plays the role of the variable. In both cases, the coefficients $a_i$ come from a specific set (a field or $\{0, \dots, p-1\}$). Just as a Laurent series converges when the variable is small, pic series converge because, under the pic metric, higher powers of $p$ are "smaller" (closer to zero). 

In standard base-$p$ arithmetic, digits to the right of the point represent $p^{-1}, p^{-2}, \dots$ and get smaller as you move right. In pics, these negative powers are actually the large components. If $k \to \infty$ in the denominator as in the sequence: 
...
For **positive integers** the digits expressed in pics or in base $p$ are identical, but the direction in which you read or sum them is different.
...
An element of the pic integers $\mathbb{Z}_p$ is an infinite formal power series:

$$\alpha = \sum_{i=0}^{\infty} a_i p^i = a_0 + a_1 p^1 + a_2 p^2 + \dots$$

For a standard positive integer $n$, the pic expansion is simply the base-$p$ expansion where all digits beyond the highest power $p^k$ are zero.
...
In the pic system $\mathbb{Z}_p$, we view numbers as infinite series of powers of $p$:
...
For **negative integers**, in pics, we don't use minus signs; we use the geometric series. For example, in $2$-adics, what is the representation of $-1$? Since $\frac{1}{1-r} = 1 + r + r^2 + \dots$, we set $r=2$:

$$-1 = \frac{1}{1-2} = 1 + 2^1 + 2^2 + 2^3 + \dots$$

In pic notation, $-1$ is written as an infinite string of the highest possible digit: $(\dots 11111)_2$ or $(\dots (p-1)(p-1)(p-1))_p$. 
...
It is critical to see that the $5$-adic expression of $4357$ is $\mathbb{F}_{5^5}.$ The $5$-adic integer $4357 \in \mathbb{Z}_5.$ If $4357$ were an element of $\mathbb{F}_{5^5}$, it wouldn't be growing "upward" into powers of $5$; it would be growing "sideways" into powers of a symbolic root $\alpha$. An element of $\mathbb{F}_{5^5}$ looks like this:
...
$A_n$ is the set of remainders modulo $5^n$. The map $\phi_n$ takes an element from $A_n$ and maps it to $A_{n-1}$ by applying the operation $\pmod{5^{n-1}}$. Example: Mapping $x_3$ to $x_2$: Let’s look at the transition from level $3$ ($A_3 = \mathbb{Z}/125\mathbb{Z}$) to level $2$ ($A_2 = \mathbb{Z}/25\mathbb{Z}$): 
...
The Hasse Principle says that for quadratic forms (equations where every term is squared, like $ax^2 + by^2 + cz^2 = 0$), the local truths (the pic and real solutions) are so restrictive that they "trap" the global Truth (the rational solution). The reason this principle works specifically for quadratic forms is that they represent "conic sections" (circles, ellipses, hyperbolas). Geometric objects of degree $2$ have a special property: if you find one point on the curve, you can find all of them by drawing lines through that point. 

There is a deep law called the **Hilbert Symbol** and the **Product Formula**. It states that for a rational number to exist, the "local signatures" of the equation in all pic worlds must multiply together to equal $1.$ If the pic worlds "agree" (the product is $1$), the rational solution exists. If they "disagree" (the product is $-1$), you can't glue them together. The Hasse Principle for quadratic forms works because there is no "hidden room" for a contradiction. If it looks like a solution everywhere, it is a solution.
...
In algebraic geometry, we call $\mathbb{F}_p$ the **residue field** of $\mathbb{Z}_p$. We can think of $\mathbb{Z}_p$ as a microscope. The highest level view (the lowest magnification) is $\mathbb{F}_p$. As we look closer (mod $p^2$, mod $p^3$), you see more digits of the pic number. The isomorphism $\mathbb{F}_p \cong \mathbb{Z}_p / p\mathbb{Z}_p$ says that the coarsest possible view of a pic integer is exactly the same thing as an element of a finite field.
...
The base cylinder represents the space of all possible $5$-adic integers before you apply any filters or constraints. It contains every number that can be written as: $$x = d_0 + d_1(5) + d_2(25) + \dots$$ where $d_i \in \{0, 1, 2, 3, 4\}$. Because it is level $0,$ no digits have been decided yet.  It is the unit ball $B_1(0).$ In pic geometry, this cylinder is the unit ball. It has a pic radius of $1$. Every point inside this silver cylinder has a pic norm $|x|_5 \le 1$. By making it transparent, you are visually showing that the "true" action (the solution to your equation) is happening inside it, as you zoom into smaller and smaller sub-cylinders.

Think of a pic integer $x \in \mathbb{Z}_p$ as a high-resolution image. The full image is an infinite string of digits:$$x = a_0 + a_1p + a_2p^2 + a_3p^3 + \dots$$ When you "blur" the image, you lose the fine details. Initially, you ignore everything that is a multiple of $p$. All you can see is $a_0$. This is $\mathbb{F}_p$. At a second step, you ignore everything that is a multiple of $p^2$. You can now see $a_0 + a_1p$. This is $\mathbb{Z}/p^2\mathbb{Z}$. At level $\infty$ you see every single digit. This is the full pic integer $\mathbb{Z}_p$. By taking the quotient $\mathbb{Z}_p / p\mathbb{Z}_p$, you are mathematically "masking" the fine details.
...
We are building the pic number $x = (2, 1, 2, \dots)$ specifically because that number is the solution to $x^2 + 1 = 0$. In the pic metric, as we add more cylinders (digits), the "error" ($x^2 + 1$) gets smaller and smaller ($5^{-1}, 5^{-2}, 5^{-3} \dots$). In summary: We solve in $\mathbb{F}_5$ to find the "main branch" of the tree. Without that first step, we wouldn't know which of the 5 colors to start painting.

To increase the "resolution," we are going to calculate and plot the Hensel Lift for $x^2 + 1 = 0$ in $\mathbb{Z}_5$ up to three levels of precision ($5^0, 5^1, 5^2$). We are using the Newton-Raphson style formula for pics:$$x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}$$ 
...
In the reals we define the value of a string by the sum $\sum d_i 5^i$. For $4444\dots_5$, that sum is $4(5^0) + 4(5^1) + 4(5^2) + \dots$. This sum diverges to $+\infty$. It never "wraps around" to $-1$ because the real line is a straight line, not a circle. In the pics we define the "size" such that $5^n \to 0$. In this world, the sum $4(5^0) + 4(5^1) + 4(5^2) + \dots$ converges to $-1$.
...
This formula is purely algebraic. It doesn't care about "smoothness"; it only cares about powers of $h$. In the pics, when we move from Level $n$ to Level $n+1$, our "nudge" is $h = d_n \cdot p^n$. Because $p^n$ is very small in pic geometry, the higher-order terms like $h^2$ ($p^{2n}$) and $h^3$ ($p^{3n}$) disappear incredibly fast! This makes the linear approximation ($f'(x)$) more accurate in pic space than it often is in real space.

If we are "lifting" a solution from $x_n$ to $x_{n+1}$, we define our nudge as: $$h = d_n \cdot p^n$$ When we look at the powers of $h$ in the Taylor expansion, we see:$h^1 = (d_n \cdot p^n)^1$: This term is what we use to solve for $d_n$. It lives exactly at the level of precision we are trying to fix. $h^2 = (d_n \cdot p^n)^2 = d_n^2 \cdot p^{2n}$: This term is "too small" to matter for our current level. In pic geometry, $p^{2n}$ is much further up the tree than $p^n$.
