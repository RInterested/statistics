...
An irrational number in $p$-adics — like the $5$-adic square root of $-1$ — has an expansion that never repeats. The gears of the odometer are constantly turning, but they never fall into a predictable loop. Each new digit to the left is a "surprise" generated by the algebraic equation we are trying to solve. We find these digits one by one using Hensel's Lemma (the $p$-adic version of Newton's Method). On the odometer, an irrational number looks like "static" or a random sequence of numbers that goes on forever to the left without a pattern.
