...
$$d(\text{roof}, \text{justfees}) \leq \max\{d(\text{roof}, \text{budget}), d(\text{budget}, \text{justfees})\}$$

Substitute the actual distances:

$$0.04 \leq \max\{0.04,  0.008\}\;\checkmark$$

Because `budget` and `just` are locked close together deep in the tree, any outside file (`roof`) looking in sees them as being at the exact same distance. The long jump up to the legal folder completely dominates the metric.
