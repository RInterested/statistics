...
While the $p$-adic integers $\mathbb{Z}_p$ consist of power series starting at $i=0$, the full field of $p$-adic numbers, denoted by $\mathbb{Q}_p$, allows for a finitary fractional part. This is where the analogy to standard decimals — and more specifically, Laurent series—becomes apparent. 

A general element $\gamma \in \mathbb{Q}_p$ is expressed as a formal Laurent series in $p$:

$$\gamma = \sum_{i=k}^{\infty} a_i p^i = a_k p^k + a_{k+1} p^{k+1} + \dots$$

where $k \in \mathbb{Z}$ is any integer (possibly negative). If $k \ge 0$, the number is a **$p$-adic integer** ($\gamma \in \mathbb{Z}_p$). If $k < 0$, the number has a fractional part, with digits appearing to the right of the radix point. $\mathbb{Q}_p$ can be viewed as the field of fractions of $\mathbb{Z}_p$. In a Laurent series, the variable is $x$ or $(z - z_0)$. In $p$-adics, the prime $p$ plays the role of the variable. In both cases, the coefficients $a_i$ come from a specific set (a field or $\{0, \dots, p-1\}$). Just as a Laurent series converges when the variable is small, $p$-adic series converge because, under the $p$-adic metric, higher powers of $p$ are "smaller" (closer to zero). In standard base-$p$ arithmetic, digits to the right of the point represent $p^{-1}, p^{-2}, \dots$ and get smaller as you move right. In $p$-adics, these negative powers are actually the large components.
...
The digits "collapse" into a repeating pattern of $1$s. This is the $p$-adic representation of $-1/2$:

$$-1/2 = \dots 11111_3$$

But we are looking for positive $1/2$. Since $1/2 = 1 + (-1/2)$, we simply add $1$ to our infinite string of $1$s:

$$(\dots 11111_3) + 1 = \dots 11112_3$$
...
### Two's Complement Representation in Computer science and the Radix:
...
The $p$-adic integers ($\mathbb{Z}_p$) are best visualized as a toroidal solenoid — an infinite "snake-in-a-snake" construction:
