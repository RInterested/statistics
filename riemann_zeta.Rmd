...
- Case A: The pure sink ($R = 1$): The field is $f(z) = \frac{1}{z}$. Applying the Pólya conjugate $\vec{V} = (u, -v)$:

$$\vec{V} = \left( \frac{x}{x^2+y^2}, -\left[ \frac{-y}{x^2+y^2} \right] \right) = \left( \frac{x}{x^2+y^2}, \frac{y}{x^2+y^2} \right)$$
...
We get the pure circulation field.3. 
...
The Complex Residue $R = a + bi$ is the controller. When we calculate the contour integral $\oint \frac{R}{z} dz = 2\pi i(a + bi)$, the distributive property of $i$ performs a beautiful physical sorting:
