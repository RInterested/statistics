...
$$\mathbf a_t = \widetilde{C} = \begin{bmatrix}\mathbf W_a&\mathbf U_a\end{bmatrix} \cdot \begin{bmatrix} \mathbf x_t \\ h_{t-1}\end{bmatrix}  + \mathbf b_a$$
...
Since there are no additional layers, there is no error from layers on top to add up to this value. If it weren't the last layer, we'd need to add it: it is as though we were imputing "blame" to the output of a layer for the bad "karma" it has contributed upstream. So in the layer $t - 1$ the error will be:

$$\Delta h_{t-1}=\Delta_{t-1}\Delta_{\text{out}_{t-1}}=0.03631-0.01828=0.01803$$

---

Backpropagating this error to the cell state, $C_t,$

$$\begin{align} h_t &= \tanh\left( C_t   \right)\odot o_t\\[2ex] 
   &= \tanh\left(a_t \odot i_t + f_t \odot C_{t-1}\right) \odot o_t
   \end{align}$$
   
We can get the 

$$\frac{\partial}{\partial C_t} J = \Delta_t \odot \left( 1 - \tanh^2\left( C_t \right)\right) \odot o_t + \frac{\partial}{\partial C_{t+1}} \odot f_{t+1}$$
