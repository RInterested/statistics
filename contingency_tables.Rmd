...
<br>
```{r}
Expected_Probabilities <- as.matrix(c(0.301,0.176,0.125,0.097,0.079,0.067,0.058,0.051,0.046))
...
We reject when the test statistic is too large, and being that $\chi^2$ on a $2$ x $2$ contingency table (i.e. with 1 df) is the same as a z test squared, we will reject at around $qnorm(0.975)^2  ~ 3.8$. The alternative is always two sided because of the squared makes it non-directinal (we simply test if the proportions are different), and when setting up the alpha risk we don't divide by two.
